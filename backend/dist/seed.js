...
const seedData = async () => {
    try {
        if (mongoose_1.default.connection.readyState === 0) {
            await mongoose_1.default.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/pet_management_system');
        ...
        try {
            const existingUser = await userModel_1.default.findOne({ email: userEmail });
            if (!existingUser) {
                await userModel_1.default.create({
                    name: 'Bhavesh Patil',
                    email: userEmail,
        ...
        // Create Pets
        try {
            const petCount = await petModel_1.default.countDocuments();
            if (petCount === 0) {
                const pets = [
                    {
                        name: 'Buddy',
                ...
                await petModel_1.default.insertMany(pets);
